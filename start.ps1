...
try {
    docker compose -f "$rootDir/infra/docker/docker-compose.yml" up -d minio postgres
} catch {
...
Start-Process powershell -WorkingDirectory "$rootDir\backend" -ArgumentList "-NoExit", "-Command", "& .\venv\Scripts\python.exe -m uvicorn app.main:app --reload --host 0.0.0.0 --port 4005"
